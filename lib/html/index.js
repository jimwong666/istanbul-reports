...
function detailTemplate(data, incrementLines = []) {
	const lineNumbers = new Array(data.maxLines).fill().map((_, i) => i + 1);

	const lineLink = (num) => {
		if (incrementLines.length === 0) {
			return `<a name='L${num}'></a><a href='#L${num}'>${num}</a>`;
		} else {
			const isIncrementLine = incrementLines.some((lines) => {
				const linesArr = lines.split(",");

				return (
					Number(linesArr[0]) <= num &&
					num <= Number(linesArr[1]) + Number(linesArr[0]) - 1
				);
			});

			return isIncrementLine
				? `<a name='L${num}'></a><a href='#L${num}'>${num}<span style="display: inline-block;width: 7px;color: #fff;font-weight: bold;background: red;text-align: center;margin-left: 5px;height: 7px;border-radius: 3.5px;">变更行</span></a>`
				: `<a name='L${num}'></a><a href='#L${num}'>${num}<span style="display: inline-block;width: 7px;color: #fff;font-weight: bold;text-align: center;margin-left: 5px;height: 7px;"></span></a>`;
		}
	};
	...
	onDetail(node, context) {
		...
		cw.write(
			detailTemplate(
				...
				node.getFileCoverage().data.incrementLines,
			),
		);
		cw.write("</table></pre>\n");
		cw.write(footerTemplate(templateData));
